...
class MinimapControl implements IControl {
  ...
  constructor(config: MiniMapOptions = {}) {
    ...
    (Object.keys(config) as Array<keyof MiniMapOptions>).forEach((_key) => {
      ...
      if (
        typeof this._options[_key] === "object" &&
        typeof config[_key] === "object"
      ) {
    ...
    if (config.collapsedWidth) {
      if (!CSS.supports("width", config.collapsedWidth))
        config.collapsedWidth = "29px";
    }
    if (config.collapsedHeight) {
      if (!CSS.supports("height", config.collapsedHeight))
        config.collapsedHeight = "29px";
    }
    if (config.borderRadius) {
      if (!CSS.supports("border-radius", config.borderRadius))
    ...
    this.Minimap = new Proxy(maplibregl.Map, {
      construct: (target, args) => {
        ...
        instance.on = new Proxy(instance.on, {
          apply: (target, thisArg, argArray) => {
            this._cleanupCallbacks.push(() =>
              instance.off(argArray[0], argArray[1], argArray[2])
            );
        ...
        instance.remove = new Proxy(instance.remove, {
          apply: (target, thisArg, argArray) => {
            for (let callback of this._cleanupCallbacks) {
              callback();
            }
            return Reflect.apply(target, thisArg, argArray);
          },
  ...
  _createMinimapContainer(parentMap: maplibregl.Map): HTMLDivElement {
    ...
    const { 1: widthValue, 2: widthUnit } = this._options.width.match(
      /(\d+)(.*)/
    ) ?? [, "50", "px"];
    const { 1: heightValue, 2: heightUnit } = this._options.height.match(
      /(\d+)(.*)/
    ) ?? [, "50", "px"];
    ...
    this._cleanupCallbacks.push(() =>
      minimapEl.removeEventListener("contextmenu", preventDefault)
    );
  ...
  _load(): void {
    ...
    if (this._options.interactions) {
      ...
      (Object.keys(interactions) as Array<keyof MinimapInteractions>).forEach(
        (i) => !interactions[i] && this._minimap[i].disable()
      );
    }

    if (this._options.minimizableMinimap) {
      const minimap_position = (
        Object.entries(this._parentMap._controlPositions) as Array<
          [ControlPosition, HTMLElement]
        >
      ).find(
        ([_, el]: [unknown, HTMLElement]) =>
          el === this._minimapContainer.parentElement
      )?.[0];

      if (!minimap_position) {
        throw new Error(
          "Toggle button wasn't created due to a missing/invalid variable `minimap_position`"
        );
      }
  ...
  _update(): void {
    ...
    if (
      this._options.minimizableMinimap &&
      this._minimapContainer.classList.contains("minimized")
    )
      return;
  ...
  _syncZoom(): void {
    ...
    this._minimap.setZoom(
      this._parentMap.getZoom() - this._options.zoomLevelOffset
    );
  }
